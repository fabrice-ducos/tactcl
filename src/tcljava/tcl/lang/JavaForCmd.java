 ...
 * WARRANTIES.
 *
 * RCS: @(#) $Id: JavaForCmd.java,v 1.5 2002/12/30 22:49:24 mdejong Exp $
 *
 */
...
public class JavaForCmd implements Command
{
	    ...
	    throws TclException
    {
        ...
        if (TclList.getLength(interp, type_var) != 2) {
            throw new TclException(interp, "invalid type_var \"" +
                type_var + "\"");
        }
        ...
        if (isArrayVarname(varname) || varname.indexOf("::") != -1) {
            throw new TclException(interp, "invalid type_var \"" +
                type_var + "\", varname must be simple scalar");
        }
        ...
        while ( true ) {
            ...
            switch (typeId) {
                ...
                case ARRAY_BOOLEAN: {
                    ...
                    wrapper = TclInteger.newInstance(
                        (booleanIndexValue ? 1 : 0));
                    break;
                }
                case ARRAY_LONG: {
                    longIndexValue = longArray[i];
                    // Can't represent long as TclInteger
                    wrapper = TclString.newInstance(
                        String.valueOf(longIndexValue));
                    break;
                }
                case ARRAY_FLOAT: {
                    floatIndexValue = floatArray[i];
                    wrapper = TclDouble.newInstance(
                        (double) floatIndexValue);
                    break;
                }
                case ARRAY_DOUBLE: {
